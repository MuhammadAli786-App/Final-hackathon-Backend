...
async function main() {
  ...
  const patResp = await fetch(`https://heroic-sparkle.railway.app/api/patients/${data.user._id}`, {headers:{Authorization:'Bearer '+token}});
  console.log('GET /patients/:id ->', patResp.status);
  console.log(await patResp.text());
  
  const apptsResp = await fetch(`https://heroic-sparkle.railway.app/api/appointments?patientId=${data.user._id}`, {headers:{Authorization:'Bearer '+token}});
