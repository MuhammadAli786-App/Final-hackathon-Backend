...
async function run() {
  ...
  const res1 = await fetch('https://heroic-sparkle.railway.app/api/patients', { headers:{Authorization:'Bearer '+token}});
  ...
  if (res1.status === 200) {
    ...
    if (arr.length > 0) {
      const id = arr[0]._id;
      const res2 = await fetch('https://heroic-sparkle.railway.app/api/patients/'+id, { headers:{Authorization:'Bearer '+token}});
